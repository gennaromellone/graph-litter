...
def segmentImage(image, b_mask, clusters=3, useMask=True):
    if useMask:
        ...
        try:
            # Using maskSLIC
            maskSLIC = maskSLIC & b_mask.astype(bool)
            segments = segmentation.slic(image, n_segments=clusters, mask=maskSLIC, compactness=10)

        except:
            # Otherwise use only SLIC
            print("Mask not working!")
            segments = segmentation.slic(image, n_segments=clusters, mask=b_mask, compactness=10)
    else:
        segments = segmentation.slic(image, n_segments=clusters, mask=b_mask, compactness=10)
    
    return segments

def segmentsFeaturesExtraction(cropped_region, segments, modelTransformer, segment_classID, idxStart = 0):
    idx = idxStart
    rois = []
    ...
    for segment_id in np.unique(segments):
        ...
        # PUT IF To exclude roi with all pixels with value 0
    ...
    for file in tqdm(files_sorted, desc='Extracting ROIs and Features', unit='img'):
        ...
        for obj_i in objects:
            ...
            if int(obj_i['id']) <= args.classes and classCounter[int(obj_i['id'])-1] < maxImages:
                ...
                print("counter", classCounter)
                # YOLO phase
                # Take only the first element
                results = yoloModel(path)
                r = results[0]
                if len(r) == 0:
                ...
                # Iterate on all detected objects
                for res in r:
                    class_id = int(res.boxes.cls.item())

                    if class_id == int(obj_i['id'])-1:
                        id_x += 1
                        ...
                        x1, y1, x2, y2 = res.boxes.xyxy.cpu().numpy().squeeze().astype(np.int32)
                        # Crop image to object region
                        isolated_object = isolated[y1:y2, x1:x2]

                        # Extract ROIs
                        object_segments = segmentImage(isolated_object, b_mask[y1:y2, x1:x2], clusters=args.clusters, useMask=False)
                        #object_segments = segmentation.slic(isolated_object, n_segments=args.clusters, mask=b_mask[y1:y2, x1:x2], compactness=10)
                        
                        if args.useBackground:
                            #TODO INVESTIGATE ON BACKGROUND WITH MASK (EXCLUDING BLACK COLOR)!!!!
                            ...
                            background_segments = segmentImage(background, b_mask, clusters=args.clusters, useMask=False)

                        
                        #frame = utility.resizeImage(frame, 800, 600)            
                        utility.exportROIimage(background, background_segments, 'roi_segments.png')
                        exit()
                        full_rois = []
                        
                        #utility.exportROIimage(background, background_segments, 'roi_background.png')
                        #utility.exportROIimage(isolated_object, object_segments, 'roi_yolo.png')
                        ...
                        if args.useBackground:
                            ...
                            full_rois += segmentsFeaturesExtraction(background, background_segments, modelTransformer, 0)
                            
                        full_rois += segmentsFeaturesExtraction(isolated_object, object_segments, modelTransformer, obj_id, len(full_rois))
